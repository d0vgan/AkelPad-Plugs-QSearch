...
static void qsShowFindResults_LogOutput_Init(const wchar_t* cszFindWhat, tDynamicBuffer* pBuf, tDynamicBuffer* pResultsBuf, DWORD dwFindAllFlags, const EDITINFO* pEditInfo)
{
    ...
    if ( (g_Options.dwFindAllResult & QS_FINDALL_RSLT_MINIMALMODE) == 0 )
    {
  ...
  UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    switch ( uMsg )
    {
        case WM_COMMAND:
        {
            ...
            else if ( id == IDM_FINDALL_MINIMALMODE )
            {
                if ( g_Options.dwFindAllResult & QS_FINDALL_RSLT_MINIMALMODE )
                    g_Options.dwFindAllResult -= QS_FINDALL_RSLT_MINIMALMODE;
                else
                    g_Options.dwFindAllResult |= QS_FINDALL_RSLT_MINIMALMODE;
            }
        ...
        case WM_CONTEXTMENU:
        {
            ...
            if ( (g_QSearchDlg.hBtnFindAll == (HWND) wParam) && !g_Plugin.bOldWindows )
            {
                ...
                uCheck = (g_Options.dwFindAllResult & QS_FINDALL_RSLT_MINIMALMODE) ? MF_CHECKED : MF_UNCHECKED;
                CheckMenuItem( g_QSearchDlg.hFindAllPopupMenu, IDM_FINDALL_MINIMALMODE, MF_BYCOMMAND | uCheck );
